...
- GitHub Actions runs `scripts/release-charts.sh` on push to `main` (or manual `workflow_dispatch`).
- The script releases charts in dependency order (topological sort), auto-bumps dependents when a dependency version changes, and commits bumped `Chart.yaml` files with `[skip ci]`.
- A `lint` job runs `helm lint` on all charts before release.
- `.tgz` packages are published as GitHub Releases.
- `index.yaml` on `gh-pages` is auto-updated.
- A pre-commit hook auto patch-bumps chart versions when chart files are staged.

## Development conventions

- **Never add `Co-Authored-By` trailers or any attribution trailers to commit messages.**
...
- generic-app `progressDeadlineSeconds`: optional top-level value to override the Argo Rollout default (600s). Must be higher than the startup probe total window (`initialDelaySeconds + failureThreshold * periodSeconds`) to avoid premature Degraded status.
- **NEVER manually bump chart versions in `Chart.yaml`** — a pre-commit hook auto patch-bumps when chart files are staged.
- **Never emit duplicate `env` names in a container.** `spec.template.spec.containers[].env` has `patchMergeKey: name` in the K8s API, so a strategic-merge apply resolves entries by name. Two entries with the same name (the pre-2.5.1 worker template concatenated `app.env` + `worker.env`, so `APP_ROLE` appeared twice) make the applied object diverge from the rendered manifest and leave the ArgoCD app permanently OutOfSync. Fixed in 2.5.1 via `mergeOverwrite`.
  - **The 2.5.1 upgrade needs TWO deploys.** Rendering correctly does not normalize objects already applied with the duplicate. On the first deploy after the bump, the merge between the live object (two `APP_ROLE` entries) and the desired one (a single entry) drops the key entirely — the live container ends up with **no** `APP_ROLE` at all. Real diff, not a phantom: ArgoCD stays OutOfSync and any `argocd app wait --synced` hangs to timeout (exit 20). The next deploy re-applies cleanly and converges. Confirmed on djv-tivy-backend-prod (2026-07-28 fail → 2026-07-30 converge, worker rev 4 had 20 env vars without `APP_ROLE`, rev 5 has 21 with `APP_ROLE=worker`).
  - If it happens again and you can't wait for a second deploy, force it with `kubectl replace` on the Deployment (or delete + resync). Adding `ServerSideApply=true` to the app's `syncOptions` would make these merges deterministic — not adopted yet.
